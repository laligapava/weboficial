...
(function () {
  ...
  if (reduce) {
    ...
    var perksCarousel = document.querySelector('.perks-carousel');
    var perksFollowers = perksCarousel ? [].slice.call(document.querySelectorAll('.btn-hint, .acc-panel-in > a.btn.reveal-blur')) : [];
    var otherReveals = revealEls.filter(function (el) { return evRows.indexOf(el) === -1 && perksFollowers.indexOf(el) === -1; });
    otherReveals.forEach(function (el) { io.observe(el); });
    if (perksCarousel) {
      var perksIo = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) {
          if (e.isIntersecting) {
            e.target.classList.add('in');
            perksFollowers.forEach(function (el) { el.classList.add('in'); });
            perksIo.unobserve(e.target);
          }
        });
      }, { threshold: 0.08, rootMargin: '0px 0px -8% 0px' });
      perksIo.observe(perksCarousel);
    }
    ...
    function sweepReveal() {
      ...
      otherReveals.concat(perksFollowers).forEach(function (el) {
  ...
  /* ---------- Same offset when arriving from another page with a #hash ---------- */
  if (window.location.hash) {
    var arrivalHash = window.location.hash;
    window.addEventListener('load', function () {
      scrollToHashTarget(arrivalHash, false);
      setTimeout(function () { scrollToHashTarget(arrivalHash, false); }, 300);
    });
  }
